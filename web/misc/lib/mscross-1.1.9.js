...
function msMap(DivTag, ControlType, p_protocol) {
  ...
  */
  ...
  } else { _tagMain.style.setProperty("-moz-user-select", "none", ""); }
  ...
  this.addPointOverlay = function (p_point, p_redraw) {
    ...
    if ((p_redraw == null) || (p_redraw == true)) { i.overlayPointsResort() }
  }
  ...
  this.addClickTool = function () { //xxx
  ...
  this.xReal2pixel = function (p_x) { return Math.round(_map_w * (p_x - _ext_Xmin) / (_ext_Xmax - _ext_Xmin)); }
  this.yReal2pixel = function (p_y) { return Math.round(_map_h * (_ext_Ymax - p_y) / (_ext_Ymax - _ext_Ymin)); }
  ...
  this.setFullExtent = function (Xmin, Xmax, Ymin) {
    ...
    if (_read_cookie == false) { i.fullExtentNoRedraw(); }	// 20060316 (IE bugfix)
  }

  this.setZoomboxFirst = function (x, y) { _zoombox_x_first = x; _zoombox_y_first = y; }
  ...
  //     var x_pct = 1 - (click_x / myMap1.width());
  //     var y_pct = 1 - (click_y / myMap1.height());  

  //     var x_map = coor[0] + ((coor[1] - coor[0]) * x_pct);
  //     var y_map = coor[2] + ((coor[3] - coor[2]) * y_pct);
  ...
  this.fullExtent = function () { i.fullExtentNoRedraw(); i.redraw(); _tagMap.style.cursor = "move"; }

  this.setActionZoombox = function () { //_action = 'zoom';
    _tagMap.style.cursor = "crosshair";
  }

  this.setActionPan = function () { //_action = 'pan';
    _tagMap.style.cursor = "move";
  }

  this.setActionCoords = function () { //_action = 'coords';
    _tagMap.style.cursor = "crosshair";
  }
  ...
  this.setActionCoordsSelect = function () {
    _tagMap.style.cursor = 'url("misc/img/pasador-de-ubicacion.png"), auto';
  }
  ...
  this.setActionZoomOut = function () {
    //_tagMap.style.cursor = "";
    if (isNaN(_tagLoading.style.display)) { i.zoomPerc(0.30); i.redraw(); }
  }
  ...
  this.mapLoaded = function () {
    ...
    for (var j = 0; j < _pointsOverlayArray.length; j++) { _pointsOverlayArray[j].redraw(); }
  ...
  this.recalc_map_size = function () {
    ...
    if (_pixel_w > _pixel_h) { // Modify only Y (box width > height)
    ...
    } else { // Modify only X (box width < height)
  ...
  this.getCookieName = function () { return '_mscross_' + hex_md5(window.location.href + DivTag.id); }
  ...
  this.init = function () {
    ...
    if (myLink.setAttribute) { myLink.setAttribute('href', ref); } else { myLink.href = ref }
  ...
  this.ubiCar = function (event) {
    ...
    let observaciones = document.getElementById("coordenadas");
    if (observaciones) {
      observaciones.value = `Coordenadas del mapa:\n${c1}\n${c2}`;
    }
    ...
    let modalB = document.getElementById('modalSolicitud');
    if (modalB) {
      var modal = new bootstrap.Modal(modalB);
      modal.show();
    }
  };
  ...
  this.zoomGo = function (event) {
    ...
    if (browser.isNS) { event.preventDefault(); } else {
      window.event.cancelBubble = true;
      window.event.returnValue = false;
    }
  }
  ...
  this.overlayPointsResort = function () {
    ...
    for (var j = 0; j < _pointsOverlayArray.length; j++) { _tagPoints.appendChild(_pointsOverlayArray[j].getShd()); }
    ...
    for (var j = 0; j < _pointsOverlayArray.length; j++) { _tagPoints.appendChild(_pointsOverlayArray[j].getImg()); }
  ...
  this.getExtent = function () {
    //_ext_Xmin, _ext_Xmax, _ext_Ymin, _ext_Ymax)
    return _ext_Xmin + "," + _ext_Xmax + "," + _ext_Ymin + "," + _ext_Ymax;
  }
...
function setAlphaBackgroundPNG(p_Tag, p_src) {
  ...
  } else { p_Tag.style.backgroundImage = "url('" + p_src + "')"; }
}



function setZindex(p_tag, p) {
  if (p_tag.setAttribute) { p_tag.setAttribute('style', 'z-index:' + p + ';'); } else { p_tag.style.zIndex = p; }
}
...
function msTool(p_title, p_event_button, p_icon, p_event_map) {
  ...
  if (p_event_button != null) { _event_button = function (e) { p_event_button(e, _map); } }
  ...
  this.removeMapEvent = function () {
    if (_event_map != null) { del_event(_map.getTagEvent(), "mousedown", _event_map, false); }
  }
}
...
function msToolbar(p_msMap, _control, _default) {
  ...
  var _iconUbiButton = 'misc/img/ubicacion.png';

  this.getTag = function () { return _tagToolbar; }
  this.hide = function () { _tagToolbar.style.display = 'none'; }

  this.removeMapEvents = function () { for (i = 0; i < _toolbarArray.length; i++) { _toolbarArray[i].removeMapEvent(); } }
  ...
  this.redraw = function () {
    if ((_control == 'standard') ||
      ...
      for (i = 0; i < _toolbarArray.length; i++) { setPos(_toolbarArray[i].getTag(), parseInt(box.style.left) + 5, (i * 40) + 5); }
    }
  ...
  if (_default == true) {
    var t_fullext = new msTool('Full Extent', _msMap.fullExtent, _iconFullExtentButton, function (e, map, x, y) { map.dragStart(e); });;
    ...
    var t_ubicacion = new msTool('Ubicacion', _msMap.setActionCoordsSelect, _iconUbiButton, function (e, map, x, y) { map.ubiCar(e) });
